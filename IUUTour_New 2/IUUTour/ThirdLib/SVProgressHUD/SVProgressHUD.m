...
UInt64 svhudShowTimestamp = 0;
NSString *currentOwner  = nil;
...
+ (void)showWithOwner:(NSString*)ownerId
{
    currentOwner = ownerId;
    svhudShowTimestamp = [[NSDate date] timeIntervalSince1970] * 10;
...
+ (void)dismissFromOwner:(NSString*)ownerId
{
    if ([self isVisible])
    {
        //  owner transferred to others...
        if (![currentOwner isEqualToString:ownerId])
        {
            return;
        }
        
        UInt64 currentTime = [[NSDate date] timeIntervalSince1970] * 10;
        int elapsedTime = (int)(currentTime - svhudShowTimestamp);
        if ( elapsedTime > 5)
        {
            [self dismissWithDelay:0];
        }
        else
        {
            ...
            {
                float waitTime = 0.5f - elapsedTime / 10.0f;
                [self performSelector:@selector(dismiss) withObject:nil afterDelay:waitTime];
            }
        }
    }
}
...
#pragma clang diagnostic pop
    ...
    //  Comment by Vincent on Sep 19 2015
    //  Don't call the following line for transparent hudView
                     ...
                     completion:^(BOOL finished){
                         ...
                         if(strongSelf){
                             if(strongSelf.alpha == 0.0f || strongSelf.hudView.alpha == 0.0f){
                                 ...
                                 [strongSelf->_hudView removeFromSuperview];
                                 strongSelf->_hudView = nil;
                                 
                                 [strongSelf->_overlayView removeFromSuperview];
                                 strongSelf->_overlayView = nil;
                                 
                                 [strongSelf->_indefiniteAnimatedView removeFromSuperview];
                                 strongSelf->_indefiniteAnimatedView = nil;
